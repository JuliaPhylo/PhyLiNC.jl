...
- `filename` ("phyLiNC"): root name for the output files (`.log`, `.err`).
...
function phyLiNC!(
    ...
    nreject=75::Int,
    nruns=10::Int,
    filename="phyLiNC"::AbstractString,
    verbose=true::Bool,
    seed=0::Int,
    probST=0.5::Float64,
    ftolRel=1e-6::Float64,
    ftolAbs=1e-6::Float64,
    xtolRel=1e-10::Float64,
    xtolAbs=1e-5::Float64,
    constraints=TopologyConstraint[]::Vector{TopologyConstraint},
    alphamin=alphaRASmin::Float64,
    alphamax=alphaRASmax::Float64,
    pinvmin=pinvRASmin::Float64,
    pinvmax=pinvRASmax::Float64
)
    writelog = true
    ...
    logstr = """complete.
      ...
      ---------------------
      NOTE: This network should be interpreted as semidirected (unrooted) because the
      root is not identifiable. It can be rooted with an outgroup or other external information.
      ---------------------
      """
    if writelog
        write(logfile, logstr)
...
to be performed is in `PhyLiNC.moveweights_LiNC`.
...
function nni_LiNC!(
    ...
    while !edgefound
        ...
        savededges = PN.adjacentedges(e1)
...
- The SSM object has cache arrays of size large enough, that is,
  the constructor `PhyloTraits.StatisticalSubstitutionModel` was previously
...
julia> obj = PhyLiNC.StatisticalSubstitutionModel(net, fastafile, :JC69; maxhybrid=3);

julia> PhyLiNC.checknetwork_LiNC!(obj.net, 3, true, true);
...
function updateSSM_priorltw!(obj::SSM)
    ...
    for (i,t) in enumerate(obj.displayedtree)
        @inbounds pltw[i] = PN.inheritanceWeight(t)
    end
    return nothing
end

# updates displayed trees when one gamma changes
function updatedisplayedtrees!(
    trees::Vector,
    enum::Int,
    pnum::Int,
    gammae::Float64,
...
julia> obj = PhyLiNC.StatisticalSubstitutionModel(net, fastafile, :JC69);
...
julia> PhyLiNC.optimizelocalBL_LiNC!(obj, e, PhyLiNC.CacheLengthLiNC(obj, 1e-6,1e-6,1e-2,1e-2, 10));
...
```
"""
function optimizelocalBL_LiNC!(
    obj::SSM,
    focusedge::Edge,
    lcache::CacheLengthLiNC
)
    neighboredges = PN.adjacentedges(focusedge)
...
"""
function optimizeallgammas_LiNC!(
    obj::SSM,
    ftolAbs::Float64,
    γcache::CacheGammaLiNC,
    maxeval::Int
...
julia> PhyLiNC.optimizelocalgammas_LiNC!(obj, obj.net.edge[3], 1e-3, PhyLiNC.CacheGammaLiNC(obj));

julia> obj.net.edge[3].gamma
0.0
````
"""
function optimizelocalgammas_LiNC!(
    obj::SSM, edge::Edge,
    ftolAbs::Float64,
    γcache::CacheGammaLiNC,
    maxeval=1000::Int
...
"""
function optimizegamma_LiNC!(
    obj::SSM,
    focusedge::Edge,
    ftolAbs::Float64,
    cache::CacheGammaLiNC,
    maxNR=10::Int
)
